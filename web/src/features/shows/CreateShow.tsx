...
type Venue = { _id: string; name: string; stageWidth?: number; stageHeight?: number };

type StageNode = { id: string; label: string; x: number; y: number; color?: string };
...
export function CreateShow() {
  ...
  const venuesQuery = useQuery<Venue[]>({ queryKey: ['venues'], queryFn: () => api('/api/venues') });
  const setlistsQuery = useQuery<Setlist[]>({ queryKey: ['setlists'], queryFn: () => api('/api/setlists') });
  const plotsQuery = useQuery<StagePlotSummary[]>({ queryKey: ['stageplots'], queryFn: () => api('/api/stageplots') });
  const stagePlotQuery = useQuery<StagePlot>({ queryKey: ['stageplot', stageContextId], queryFn: () => api(`/api/stageplots/${stageContextId}`) });

  const create = useMutation({
    mutationFn: (payload: Partial<Show>) => api<Show>('/api/shows', { method: 'POST', body: JSON.stringify(payload) })
  });

  const currentVenue = useMemo(() => venuesQuery.data?.find((v) => v._id === venueId), [venuesQuery.data, venueId]);
  const stageWidthPx = Math.max(120, (currentVenue?.stageWidth || 0) * FEET_TO_PX || 300);
  const stageHeightPx = Math.max(80, (currentVenue?.stageHeight || 0) * FEET_TO_PX || 180);
  ...
  return (
    <div>
      ...
      >
        ...
        <input className="rounded-md border px-3 py-2" placeholder="Date" value={date} onChange={(e) => setDate(e.target.value)} />
        <select className="rounded-md border px-3 py-2" value={venueId} onChange={(e) => setVenueId(e.target.value)}>
        ...
        <div className="rounded-md border p-2" style={{ borderColor: 'var(--line)', background: '#fff' }}>
          ...
          <div className="mt-2">
            <div className="text-xs font-medium mb-1 opacity-70">Selected order</div>
            <ol className="list-decimal ml-5 text-sm">
              {selectedSetlists.map((id) => {
                const name = setlistsQuery.data?.find((s) => s._id === id)?.name || id;
        ...
        >
          ...
          {plotsQuery.data?.map((p) => (
            <option key={p.contextId} value={p.contextId}>{p.name || p.contextId}</option>
          ))}
        </select>
      ...
      <div className="relative rounded-md border" style={{ width: stageWidthPx, height: stageHeightPx, background: '#fff', borderColor: 'var(--line)' }}>
        {nodes.map((n) => {
          const left = Math.round(n.x * scale);
          const top = Math.round(n.y * scale);
          return (
            <div key={n.id} className="absolute px-2 py-1 rounded-md" style={{ left, top, background: n.color || '#eee', border: '1px solid rgba(0,0,0,0.2)' }}>
              {n.label}
            </div>
          );
        })}
